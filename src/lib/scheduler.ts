...
export async function checkGoalsAndNotify() {
    try {
        ...
        for (const user of users) {
            ...
            console.log(`Checking goals for user ${user.id} at 10 PM ${userTimezone}`);
            ...
            const goalsForToday = user.goals.filter(goal => {
                ...
                if (goal.recurringDays) {
                    try {
                            let days = JSON.parse(goal.recurringDays);
                            if (typeof days === 'string') days = JSON.parse(days);
                            return Array.isArray(days) && (days.length === 0 || days.includes(userDate.getDay()));
                    } catch {
                        return false;
                    }
                }
          ...
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p style="font-size: 12px; color: #777; margin-top: 24px;">Manage reminders and account preferences from Settings.</p>
          </div>
          `
                );
            }
